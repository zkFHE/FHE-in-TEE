...
#include "seal_sgx/util/ntt.h"
#include "seal_sgx/util/uintarith.h"
#include "seal_sgx/util/uintarithsmallmod.h"
#include <algorithm>
#ifdef SEAL_USE_INTEL_HEXL
#include "seal_sgx/memorymanager.h"
#include "seal_sgx/util/iterator.h"
#include "seal_sgx/util/locks.h"
#include "seal_sgx/util/pointer.h"
...
namespace intel
{
    namespace hexl
    {
        ...
        template <>
        struct NTT::AllocatorAdapter<seal_sgx::MemoryPoolHandle>
            : public AllocatorInterface<NTT::AllocatorAdapter<seal_sgx::MemoryPoolHandle>>
        {
            AllocatorAdapter(seal_sgx::MemoryPoolHandle handle) : handle_(std::move(handle))
            {}
            ...
            {
                cache_.push_back(static_cast<seal_sgx::util::MemoryPool &>(handle_).get_for_byte_count(bytes_count));
            ...
            void deallocate_impl(void *p, SEAL_MAYBE_UNUSED std::size_t n)
            {
                auto it = std::remove_if(
                    ...
                    [p](const seal_sgx::util::Pointer<seal_sgx::seal_byte> &seal_pointer) { return p == seal_pointer.get(); });
        ...
        private:
            seal_sgx::MemoryPoolHandle handle_;
            std::vector<seal_sgx::util::Pointer<seal_sgx::seal_byte>> cache_;
        };
        ...
        template <>
        struct NTT::AllocatorAdapter<seal_sgx::MemoryPoolHandle, SimpleThreadSafePolicy>
            : public AllocatorInterface<NTT::AllocatorAdapter<seal_sgx::MemoryPoolHandle, SimpleThreadSafePolicy>>
        {
            AllocatorAdapter(seal_sgx::MemoryPoolHandle handle, SimpleThreadSafePolicy &&policy)
            ...
            {
                {
                    ...
                    auto accessor = policy_.locker();
                    auto it = std::remove_if(
                        cache_.begin(), cache_.end(), [p](const seal_sgx::util::Pointer<seal_sgx::seal_byte> &seal_pointer) {
    ...
    namespace seal_sgx_ext
    {
        ...
        */
        static intel::hexl::NTT &get_ntt(size_t N, uint64_t modulus, uint64_t root)
        {
            ...
            static seal_sgx::util::ReaderWriterLocker ntt_cache_locker_;

            pair<uint64_t, uint64_t> key{ N, modulus };

            // Enable shared access to NTT already present
            {
                seal_sgx::util::ReaderLock reader_lock(ntt_cache_locker_.acquire_read());
            ...
            seal_sgx::util::WriterLock write_lock(ntt_cache_locker_.acquire_write());

            // Check ntt_cache for value (may be added by another thread)
            auto ntt_it = ntt_cache_.find(key);
            if (ntt_it == ntt_cache_.end())
            {
                hexl::NTT ntt(N, modulus, root, seal_sgx::MemoryManager::GetPool(), hexl::SimpleThreadSafePolicy{});
        ...
        */
        static void compute_forward_ntt(
            seal_sgx::util::CoeffIter operand, std::size_t N, std::uint64_t modulus, std::uint64_t root,
