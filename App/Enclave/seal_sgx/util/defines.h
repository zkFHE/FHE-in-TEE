...
// MSVC support
#include "seal_sgx/util/msvc.h"

// clang support
#include "seal_sgx/util/clang.h"

// gcc support
#include "seal_sgx/util/gcc.h"
...
#define SEAL_ITERATE seal_sgx::util::seal_for_each_n
#else
...
#define SEAL_ALLOCATE_GET_PTR_ITER(name, type, size, pool)                               \
    auto SEAL_JOIN(_seal_temp_alloc_, __LINE__)(seal_sgx::util::allocate<type>(size, pool)); \
    seal_sgx::util::PtrIter<type *> name(SEAL_JOIN(_seal_temp_alloc_, __LINE__).get());
...
#define SEAL_ALLOCATE_GET_STRIDE_ITER(name, type, size, stride, pool)                                                  \
    auto SEAL_JOIN(_seal_temp_alloc_, __LINE__)(seal_sgx::util::allocate<type>(seal_sgx::util::mul_safe(size, stride), pool)); \
    seal_sgx::util::StrideIter<type *> name(SEAL_JOIN(_seal_temp_alloc_, __LINE__).get(), stride);
...
#define SEAL_ALLOCATE_GET_POLY_ITER(name, poly_count, poly_modulus_degree, coeff_modulus_size, pool) \
    auto SEAL_JOIN(_seal_temp_alloc_, __LINE__)(                                                     \
        seal_sgx::util::allocate_poly_array(poly_count, poly_modulus_degree, coeff_modulus_size, pool)); \
    seal_sgx::util::PolyIter name(SEAL_JOIN(_seal_temp_alloc_, __LINE__).get(), poly_modulus_degree, coeff_modulus_size);
...
#define SEAL_ALLOCATE_ZERO_GET_POLY_ITER(name, poly_count, poly_modulus_degree, coeff_modulus_size, pool) \
    auto SEAL_JOIN(_seal_temp_alloc_, __LINE__)(                                                          \
        seal_sgx::util::allocate_zero_poly_array(poly_count, poly_modulus_degree, coeff_modulus_size, pool)); \
    seal_sgx::util::PolyIter name(SEAL_JOIN(_seal_temp_alloc_, __LINE__).get(), poly_modulus_degree, coeff_modulus_size);
...
#define SEAL_ALLOCATE_GET_RNS_ITER(name, poly_modulus_degree, coeff_modulus_size, pool) \
    auto SEAL_JOIN(_seal_temp_alloc_, __LINE__)(                                        \
        seal_sgx::util::allocate_poly(poly_modulus_degree, coeff_modulus_size, pool));      \
    seal_sgx::util::RNSIter name(SEAL_JOIN(_seal_temp_alloc_, __LINE__).get(), poly_modulus_degree);
...
#define SEAL_ALLOCATE_ZERO_GET_RNS_ITER(name, poly_modulus_degree, coeff_modulus_size, pool) \
    auto SEAL_JOIN(_seal_temp_alloc_, __LINE__)(                                             \
        seal_sgx::util::allocate_zero_poly(poly_modulus_degree, coeff_modulus_size, pool));      \
    seal_sgx::util::RNSIter name(SEAL_JOIN(_seal_temp_alloc_, __LINE__).get(), poly_modulus_degree);
...
#define SEAL_ALLOCATE_GET_COEFF_ITER(name, poly_modulus_degree, pool)                                  \
    auto SEAL_JOIN(_seal_temp_alloc_, __LINE__)(seal_sgx::util::allocate_uint(poly_modulus_degree, pool)); \
    seal_sgx::util::CoeffIter name(SEAL_JOIN(_seal_temp_alloc_, __LINE__).get());
...
#define SEAL_ALLOCATE_ZERO_GET_COEFF_ITER(name, poly_modulus_degree, pool)                                  \
    auto SEAL_JOIN(_seal_temp_alloc_, __LINE__)(seal_sgx::util::allocate_zero_uint(poly_modulus_degree, pool)); \
    seal_sgx::util::CoeffIter name(SEAL_JOIN(_seal_temp_alloc_, __LINE__).get());
